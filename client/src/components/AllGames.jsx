      ...
      {games.length > 0 ? (
        <section className="allGames__container">
          ...
          <div className="button__container">
            <label>
              <select
                onChange={(e) => handleOrderSelect(e.target.value, state)}
                className="select"
              >
            ...
            {state === "searchGames" && (
              <label>
                <select
                  onChange={(e) => handleFilterMyGames(e.target.value, state)}
                  className="select"
                >
        ...
        <section>
          ...
          <div className="notFound__container">
            <h4 className="notFound--title">
              Ups, no has creado ningun videojuego aun!
            </h4>
            <button
...
const mapDispatchToProps = (dispatch) => {
  return {
    sortState: (type, game) => {
      dispatch(sortState(type, game));
    },
    filter: (type, state) => {
      dispatch(filter(type, state));
    },
    filterMyGames: (type, state) => {
      dispatch(filterMyGames(type, state));
    },
  };
};
